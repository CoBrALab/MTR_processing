...
if [ "$coil_type" == "nrm" ]; then minccalc -expression 'A[0]/(1.38396756*A[1]-0.31847942)' $output/mtr_maps/$(basename -s .mnc $2)_mtr_map_minccalc.mnc $output/b1_maps/normalized_and_registered_b1/${basename}_b1_map_registered_norm.mnc $output/mtr_maps/corrected_mtr_maps/$(basename -s .mnc $2)_mtr_map_minccalc_corrected.mnc; fi
...
/data/chamal/projects/mila/2019_Magnetization_Transfer/scripts/histogram_creation.sh $output $(basename -s .mnc $2)_corr_hist.txt $output/mtr_maps/corrected_mtr_maps/$(basename -s .mnc $2)_mtr_map_minccalc_corrected.mnc $(basename -s .mnc $2)
/data/chamal/projects/mila/2019_Magnetization_Transfer/scripts/histogram_creation.sh $output $(basename -s .mnc $2)_hist.txt $output/mtr_maps/$(basename -s .mnc $2)_mtr_map_minccalc.mnc $(basename -s .mnc $2)
