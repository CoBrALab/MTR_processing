...
#as of August 2019, the script works best for scans taken on the room-temperature coil. We are not yet able to correct the surface cryocoil MTR maps due to extensive signal drop-off.
...
#perform the correction-pull the value for m and c from the python script! - is this really necessary, the final script will use a set calibration value
...
#mincstats -histogram $output/mtr_histograms/$(basename -s .mnc $2)_corrected_hist.txt -mask $output/mtr_histograms/labels_no_csf_transformed_to_${basename}.mnc -mask_binvalue 1 $output/mtr_maps/$(basename -s .mnc $2)_mtr_map_minccalc_corrected.mnc -hist_bins 200 -hist_range -1 1
