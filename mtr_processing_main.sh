...
for file in $tmp_subject_dir/*; do mouse-preprocessing-orientation.sh $file $output/preprocessed/$(basename -s .mnc $file)_processed.mnc; done
for file in $tmp_subject_dir/*; do if [[ "$file" != *b1* ]]; then mouse-preprocessing-denoise-only.sh $file $output/denoised/$(basename -s .mnc $file)_denoised.mnc; fi; done
mouse-preprocessing-mask.sh $output/preprocessed/$(basename -s .mnc $3)_processed.mnc $output/masks/${basename}_mask.mnc
for file in $output/preprocessed/*b1*; do iter_bias_cor.sh $file $output/denoised/$(basename -s .mnc $2)_denoised.mnc $output/masks/${basename}_mask.mnc $output/denoised/$(basename -s .mnc $file)_denoised.mnc; done
...
mtr_register_strong.sh $output/denoised/${basename}_b1_60_processed_denoised.mnc $output/denoised/$(basename -s .mnc $2)_denoised.mnc  $output/b1_maps/
...
mtr_register_strong.sh $path_to_no_csf_mask $output/denoised/$(basename -s .mnc $2)_denoised.mnc $output/mtr_histograms/ $output/mtr_histograms/
