...
#usage:
#mtr_processing_main.sh output_folder MT_image PD_image mask_for_calc_SNR WM_mask_for_calc_CNR
...
###################################################################Standard MTR processing and map creation ###########################################
...
#create MTR maps too
ImageMath 3 $output/mtr_maps/$(basename -s .mnc $mt)_mtr_map_imagemath.mnc MTR $output/preprocessed/$(basename -s .mnc $pd)_processed.mnc $output/preprocessed/$(basename -s .mnc $mt)_processed.mnc $output/masks/${basename}_${version}_mask_nocsf.mnc
...
echo ${basename},top,$MTR_top,$mean_pd_top,$stddev_back,$MTR_wm,$output/mtr_maps/$(basename -s .mnc $mt)_mtr_map_imagemath.mnc,$output/preprocessed/$(basename -s .mnc $pd)_processed.mnc,$mask1,$backgroundmask,$mask_wm>> $output/snr_param_opt.csv
echo ${basename},bottom,$MTR_bottom,$mean_pd_bottom,$stddev_back,$MTR_wm,$output/mtr_maps/$(basename -s .mnc $mt)_mtr_map_imagemath.mnc,$output/preprocessed/$(basename -s .mnc $pd)_processed.mnc,$mask1,$backgroundmask,$mask_wm >> $output/snr_param_opt.csv
