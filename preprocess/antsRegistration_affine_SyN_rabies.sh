...
if (( 190 > $ratio )); then
  steps_affine=$(/data/chamal/projects/mila/2019_Magnetization_Transfer/scripts/ants_generate_iterations.py --min 0.1 --max 19.0 --output multilevel-halving)
else
  steps_affine=$(/data/chamal/projects/mila/2019_Magnetization_Transfer/scripts/ants_generate_iterations.py --min ${fixed_minimum_resolution} --max ${fixed_maximum_resolution} --output multilevel-halving)
fi

steps_syn=$(/data/chamal/projects/mila/2019_Magnetization_Transfer/scripts/ants_generate_iterations.py --min ${fixed_minimum_resolution} --max ${fixed_maximum_resolution})
